...
using GltfMap = std::unordered_map<const Key*, Value>;

template <typename Key, typename Value>
const Value& Find(const Key* const gltf_key, const GltfMap<Key, Value>& gltf_map) {
  const auto iterator = gltf_map.find(gltf_key);
  assert(iterator != gltf_map.cend());  // assumed to contain all known key-value pairs before this function is called
...
std::unique_ptr<vktf::Material> CreateMaterial(const vk::Device device,
  ...
  return std::make_unique<vktf::Material>(std::move(base_color_texture),
...
vktf::Primitive CreatePrimitive(const cgltf_primitive& gltf_primitive, const vktf::Material* material) {
  ...
  switch (const auto component_size = cgltf_component_size(gltf_primitive.indices->component_type)) {
    case 2: {
      return vktf::Primitive{CreateVertices(*position_attribute.maybe_data,
                                            *normal_attribute.maybe_data,
                                            *tangent_attribute.maybe_data,
                                            *texture_coordinates_0_attribute.maybe_data),
                             UnpackIndices<std::uint16_t>(*gltf_primitive.indices),
                             material};
    }
    case 4: {
      return vktf::Primitive{CreateVertices(*position_attribute.maybe_data,
                                            ...
                                            *tangent_attribute.maybe_data,
                                            *texture_coordinates_0_attribute.maybe_data),
                             UnpackIndices<std::uint32_t>(*gltf_primitive.indices),
                             material};
    }
    default: {
...
std::unique_ptr<const vktf::Mesh> CreateMesh(const cgltf_mesh& gltf_mesh,
  ...
  for (auto index = 0; const auto& gltf_primitive : std::span{gltf_mesh.primitives, gltf_mesh.primitives_count}) {
    ...
    auto& primitive = primitives.emplace_back(CreatePrimitive(gltf_primitive, material));
    auto& [command_buffer, allocator, staging_buffers] = copy_buffer_options;
    auto [vertex_staging_buffer, index_staging_buffer] = primitive.CreateBuffers(command_buffer, allocator);
    staging_buffers.push_back(std::move(vertex_staging_buffer));
    staging_buffers.push_back(std::move(index_staging_buffer));
